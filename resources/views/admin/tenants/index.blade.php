        ...
        <div class="md:flex md:items-center md:justify-between mb-8">
            <div class="flex-1 min-w-0">
                <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">
                    {{ __('admin.companies.title') }}
                </h2>
                <p class="mt-1 text-sm text-gray-500">{{ __('admin.companies.subtitle') }}</p>
            </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('admin.companies.default_locale') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('admin.companies.status') }}</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('admin.companies.created_at') }}</th>
                                ...
                                @foreach($tenants as $tenant)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            ...
                                            @else
                                                <span class="text-gray-400">{{ __('admin.companies.no_domain') }}</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                <a href="{{ route('admin.tenants.show', $tenant) }}" 
                                                   class="text-blue-600 hover:text-blue-900" title="{{ __('admin.common.view') }}">
                                                ...
                                                <a href="{{ route('admin.tenants.edit', $tenant) }}" 
                                                   class="text-indigo-600 hover:text-indigo-900" title="{{ __('admin.common.edit') }}">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                ...
                                                <a href="https://{{ $tenant->domains->first()->domain ?? $tenant->id . '.book.aimadarek.com' }}/panel" 
                                                   target="_blank" 
                                                   class="text-green-600 hover:text-green-900" title="{{ __('admin.companies.panel') }}">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path>
                                                    </svg>
                                                </a>
                                                <form method="POST" action="{{ route('admin.tenants.destroy', $tenant) }}" 
                                                      class="inline" 
                                                      onsubmit="return confirm('{{ __('admin.companies.delete_confirmation') }}')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" 
